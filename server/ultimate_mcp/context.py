...
class HaWsClient:
    """Persistent WebSocket to ws://supervisor/core/websocket with auto-reconnect.

    Skeleton: full implementation lands in W0. call() sends a command and awaits
    the matching id response; subscribe() yields events (realtime/ surface).
    """

    def __init__(self) -> None:
        self._connected = False

    async def call(self, command: str, **kwargs: Any) -> Any:  # pragma: no cover
        raise NotImplementedError("W0: WS client implementation pending")

    async def subscribe(self, event_type: str | None = None):  # pragma: no cover
        raise NotImplementedError("W0: WS subscriptions pending")
